...
class TriageScoreInteraction(Triage):
    ...
    def rts_score(self, vitals_dict):
        if 'gcs' not in vitals_dict or 'sbp' not in vitals_dict or 'rr' not in vitals_dict:
            return None
        ...
        return 0.9368 * normalized_gcs + 0.7326 * normalized_sbp + 0.2908 * normalized_rr
    ...
    def triage(self, patients: List[Patient]) -> List[TriageScore]:
        ...
        for patient in patients:
            ...
            if not record:
                triage_scores.append(TriageScore(patient_name=record['name'], score=0.0, rationale=rationale_records,
                                                 datetime_seconds=int(datetime.now().timestamp()), algo_name="LifeTriage",
                                                 interaction=None))
                continue
            ...
            if niss_score is not None and rts_score is not None:
                life_score = 100 - 0.5 * niss_score - 7 * rts_score
                score = life_score
                algo_name = 'LIFE'
                ...
                triage_scores.append(TriageScore(patient_name=record['name'], score=0.0, rationale=[],
                                                 datetime_seconds=int(datetime.now().timestamp()), algo_name="None",
                                                 interaction=None))
                continue
    ...
    def run_triage_algo(self, triage_interaction_request: TriageInteractionRequest) -> TriageInteractionRequest:
        ...
        if triage_scores:
            ...
            triage_interaction_request.score = triage_scores[0].score  # Set the score
